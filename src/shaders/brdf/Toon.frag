...
vec3 pointCol(vec3 fragmentPos) {
	...
	return pow(lubo.lightCol.a / length(lightPos - fragmentPos), 2.0) * lubo.lightCol.rgb;
}
...
vec3 spotCol(vec3 fragmentPos) {
	vec3 lightPos = vec3(0, 10, 0);
	float cosIn = cos(60.0f), cosOut = cos(120.0f);

	float ext = clamp((dot(normalize(lightPos - fragmentPos), lubo.lightDir) - cosOut) / (cosIn - cosOut), 0.0, 1.0); // Extended light model factor.
...
void main() {
	...
	vec3 L = directDir();
	vec3 Albedo = texture(tex, fragUV).rgb;	vec3 Fun = BRDF(EyeDir, Norm, L, Albedo, vec3(1.0f));	vec3 lightCol = directCol();	vec3 Ambient = vec3(0.1f);	outColor = vec4(Fun * lightCol.rgb + Albedo * Ambient, 1.0f);}